...
{
    public partial class frmTriangularDistribution : Form
    {
        #region Private Fields

        private int _iterationNumber;
        private double _inputMinimum;
        private double _inputMode;
        ...
        public frmTriangularDistribution()
        {
            InitializeComponent();
        }
        ...
        private void frmTriangularDistribution_Load(object sender, EventArgs e)
        {
        ...
        private void GetInputs()
        {
            ...
            _inputMode = Convert.ToDouble(txtbxMode.Text);
            _inputMaximum = Convert.ToDouble(txtbxMaximum.Text);
        }

        private void GetOutputs()
        {
            _randomNumberList = RandomNumberGenerator.GetListOfTriangularDistributedRandomNumbers(_iterationNumber, _inputMinimum, _inputMode, _inputMaximum);
            _pdfList = TriangularDistribution.GetListOfPDF(_inputMinimum, _inputMode, _inputMaximum, _randomNumberList);
            _cdfList = TriangularDistribution.GetListOfCDF(_inputMinimum, _inputMode, _inputMaximum, _randomNumberList);
        }
        ...
        {
            lblMean.Text = $"Mean: {Math.Round(TriangularDistribution.GetMean(_inputMinimum, _inputMode, _inputMaximum), 2)}";
            lblStandardError.Text = $"Standard Error: ";
            lblMedian.Text = $"Median: {Math.Round(TriangularDistribution.GetMedian(_inputMinimum, _inputMode, _inputMaximum), 2)}";
            lblMode.Text = $"Mode: {Math.Round(TriangularDistribution.GetMode(_inputMode), 2)}";
            lblStandardDeviation.Text = $"Standard Deviation: {Math.Round(TriangularDistribution.GetStandardDeviation(_inputMinimum, _inputMode, _inputMaximum), 2)}";
            lblVariance.Text = $"Variance: {Math.Round(TriangularDistribution.GetVariance(_inputMinimum, _inputMode, _inputMaximum), 2)}";
